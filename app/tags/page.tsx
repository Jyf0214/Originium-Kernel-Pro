  ...
  return (
    <div className="min-h-screen flex flex-col bg-zinc-50 dark:bg-zinc-900">
      <main className="flex-1 max-w-7xl mx-auto w-full px-6 py-12 md:py-20">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {tags.map(({ name, count }) => (
                <Link
                  key={name}
                  href={`/tags/${encodeURIComponent(name)}`}
                  className="group flex items-center justify-between bg-white dark:bg-zinc-800 rounded-2xl border border-zinc-100 dark:border-zinc-700 px-5 py-3.5 hover:border-zinc-300 dark:hover:border-zinc-500 hover:shadow-lg hover:shadow-zinc-100/50 transition-all duration-300"
                >
                  <div className="flex items-center gap-2.5">
                    <Hash size={14} className="text-zinc-300 group-hover:text-zinc-600 transition-colors" />
                    <span className="font-medium text-zinc-900 dark:text-zinc-100 group-hover:text-zinc-600 transition-colors">
                      {name}
                    </span>
                  </div>
                  <span className="text-xs font-bold text-zinc-400 dark:text-zinc-500 bg-zinc-50 group-hover:bg-zinc-100 px-2.5 py-0.5 rounded-full transition-colors">
                    {count}
                  </span>
                </Link>
              ))}
        ...
        {/* 底部导航 */}
        <div className="mt-16 pt-8 border-t border-zinc-100 dark:border-zinc-700">
          <Link
            href="/posts"
